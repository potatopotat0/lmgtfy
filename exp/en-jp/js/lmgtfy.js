...
$(document).ready(function(){
    ...
    $('#search').on('click',function(){
        ...
        $.ajax({ 
            ...
            success: function (data) {
                ...
                $('#go').attr("href",link);
                $('#link').show();
                $('#instructions').text('コッピ　ザー　リンック　ビロー　アンド　センド　ゼム　イット。');
    ...
    if (!!window.location.search){
        ...
        var $arrow = $('#arrow');
        setTimeout(function (){
            $instructions.text('1、ファインド　ザー　インプット　ボクス　アンド　セレックット　イット。');
            $arrow.show().animate({
                left: $kw.offset().left + 10 + 'px',
                top: ($kw.offset().top + $kw.height()/2) + 'px'
            }, 2000, function (){
                $instructions.text('2、エンター　ヨー　クェスチョン');
                $arrow.hide();
                var $kw = $('#kw');
                $kw.focus();
                var i = 0;
                var interval = setInterval(function (){
                    ...
                    if (i > kw.length){
                        ...
                        $instructions.text('3、クリック　ザー　「グーグル」　サーチ　ボトン');
                        $arrow.show();
                        var $search = $('#search');
                        $arrow.animate({

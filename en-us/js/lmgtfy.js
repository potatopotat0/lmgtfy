...
$(document).ready(function(){
    ...
    $('#search').on('click',function(){
        ...
        $.ajax({ 
            ...
            success: function (data) {
                ...
                $('#go').attr("href",link);
                $('#link').show();
                $('#instructions').text('Copy the link below and send them it.');
                $('#lmgtfyLink').val(link).focus().select();
            }
        })
    });
    $('#search2').on('click',function(){
        if($(".search-text").attr("data-site")=="google"){window.location = 'https://www.google.com.hk/search?q=' + urlEncode($('#kw').val());}else{window.location = 'https://www.google.lolicon.work/search?q=' + urlEncode($('#kw').val());}
    });
    ...
    if (!!window.location.search){
        ...
        var $arrow = $('#arrow');
        setTimeout(function (){
            $instructions.text('1、Find the input box and select it.');
            $arrow.show().animate({
                left: $kw.offset().left + 10 + 'px',
                top: ($kw.offset().top + $kw.height()/2) + 'px'
            }, 2000, function (){
                $instructions.text('2、Enter your question.');
                $arrow.hide();
                var $kw = $('#kw');
                $kw.focus();
                var i = 0;
                var interval = setInterval(function (){
                    ...
                    if (i > kw.length){
                        ...
                        $instructions.text('3、Click the "Google Search" botton.');
                        $arrow.show();
                        var $search = $('#search');
                        $arrow.animate({
                            ...
                            $instructions.html('<strong>Is it that difficult for you?</strong>');
                            setTimeout(function (){
